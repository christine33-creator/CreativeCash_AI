...
# Build the web version using webpack
echo "Building web version with webpack..."
npx webpack --config node_modules/@expo/webpack-config/webpack.config.js --mode production
...
if [ $? -eq 0 ]; then
  ...
  # Copy files from web-build (webpack output) to web-build
  echo "Copying files to web-build..."
